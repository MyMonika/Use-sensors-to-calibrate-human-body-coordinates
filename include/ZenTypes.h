...
{
    /**
    User-readable name of the sensor device传感器设备的用户可读名称
    */
    ...
    of identification number.传感器的硬件序列号。如果IO子系统无法读取序列号，则这可以是另一种形式的标识号。
    */
    ...
    */
    ...
    /**
	 ...
	 与设备一起使用的波特率。波特率为0表示OpenZen应该使用默认波特率，或者与设备协商合适的波特率。
     */
    uint32_t baudRate;
} ZenSensorDesc;
...
typedef enum ZenImuEvent
{
    ...
    // Components are free to expose private events in this reserved region组件可以自由地公开这个保留区域中的私有事件
